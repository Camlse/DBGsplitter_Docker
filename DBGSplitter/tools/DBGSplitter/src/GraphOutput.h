...
class GraphOutput {
 private:
    ...
    string prefix;
    const Graph *graphKP1;
    const SolidKmerOracle *solidKmerOracleKP1;
    const vector<vector<int>> *edgeIndexKP1_2_CountVector;
    int nbCondRepls;
...
public:
    ...
    /*                              */
    /************************************************************************************************************************/
    GraphOutput(Graph *graph=NULL, const SolidKmerOracle *solidKmerOracle=NULL, const string &prefix="graph", const Graph *graphKP1=NULL, const SolidKmerOracle *solidKmerOracleKP1=NULL,
                const vector<vector<int>> *edgeIndexKP1_2_CountVector=NULL, int nbCondRepls=0) : graph(graph), solidKmerOracle(solidKmerOracle), prefix(prefix), graphKP1(graphKP1), solidKmerOracleKP1(solidKmerOracleKP1),
                                                                              edgeIndexKP1_2_CountVector(edgeIndexKP1_2_CountVector), nbCondRepls(nbCondRepls){}
...

    {
        ...
        //we print the basic stuff
        fprintf(edges_file,"%ld\t%ld\t%s",id,id2,label.c_str());

        if (solidKmerOracle && graphKP1 && solidKmerOracleKP1 && edgeIndexKP1_2_CountVector) {
            //we get the sequence of the nodes
            string fromSeq = (label[0] == 'F' ? solidKmerOracle->getUnitigSequence(id) : reverse_complement(
                solidKmerOracle->getUnitigSequence(id)));
            string toSeq = (label[1] == 'F' ? solidKmerOracle->getUnitigSequence(id2) : reverse_complement(
                solidKmerOracle->getUnitigSequence(id2)));

            //we get the edge sequence
            long k = solidKmerOracle->getKmerSize();
            string edgeSeq = fromSeq.substr(fromSeq.size() - k, k) + toSeq.substr(k - 1, 1);

            //we get the count vector associated with this (k+1)-mer
            vector<int> zeroCountVector(nbCondRepls, 0); //TODO: make this static
            const vector<int>* countVector=NULL;

            try {
                //check if the (k+1)-mer exists
                solidKmerOracleKP1->contains(edgeSeq);
                auto edge = graphKP1->buildNode(edgeSeq.c_str());
                auto edgeIndex = graphKP1->nodeMPHFIndex(edge);

                //it does, get the associated count vector
                countVector = &((*edgeIndexKP1_2_CountVector)[edgeIndex]);
            }catch (...) {
                //it does not, get the zero count vector
                countVector = &zeroCountVector;
                cout << endl << "[WARNING]: edge " << edgeSeq << " is not present in the (k+1)-mers, so it is quantified as 0." << endl;
            }

            //we print the counts
            for (int count : (*countVector))
                fprintf(edges_file, "\t%d", count);
        }
        fprintf(edges_file, "\n");
    }
    ...
    void load_nodes_extremities(const string &linear_seqs_name, bool verbose=false)
    {
        ...
        Iterator<Sequence>* sequenceIt = Nodes->iterator();
        ProgressIterator<Sequence> progressIt(*Nodes, "Loading endpoints of unitigs");
        Iterator<Sequence> *it = NULL;
        if (verbose)
            it = &progressIt;
        else
            it = sequenceIt;
    ...
    void construct_graph(string linear_seqs_name, bool verbose=false)
    {
        ...
        // for each node, output all the out-edges (in-edges will correspond to out-edges of neighbors)
        Iterator<Sequence>* sequenceIt = Nodes->iterator();
        ProgressIterator<Sequence> progressIt(*Nodes, "Building .nodes and .edges files");
        Iterator<Sequence> *it = NULL;
        if (verbose)
            it = &progressIt;
        else
